...
│       └── requirements.txt  ← anthropic, google-generativeai, reportlab, requests
...
| FastAPI + BackgroundTasks | `handler(request)` routes, `_handle_analyze` spawns a plain `threading.Thread` and returns immediately — see "Pipeline flow" below |
| `is_cancelled(job_id)` reads dict | `_is_cancelled(context, job_id)` reads DataStore |
| WeasyPrint for PDF | ReportLab (Platypus) directly — not xhtml2pdf, see `generate_pdf_report()`'s header comment |
...
12. generate_pdf_report(result, subject, initiated_by)  ← ReportLab (Platypus)
...
Generated by `generate_pdf_report()` directly against ReportLab's Platypus
API (`Table`/`TableStyle`/`Paragraph`/`HRFlowable`) — NOT xhtml2pdf's HTML/CSS
translation layer, which had no flexbox support at all and unreliable table
column width handling (confirmed live: currency cards ran together with no
spacing, the Status column ran off the page edge). ReportLab is already
xhtml2pdf's own PDF-drawing backend, so this costs zero extra dependency
weight — see the function's own header comment and DEPLOYMENT.md changelog
for the full story, and its "What NOT to change" note on base-14 PDF fonts
not supporting glyphs like ✓ ⚠ ↔ (deliberately using ASCII equivalents
instead of embedding a Unicode font for a handful of decorative symbols).
...
| Symptom | Cause | Fix |
|---|---|---|
| `libpango` error | WeasyPrint needs C libs | Historical — moot now that PDF generation is native ReportLab (no HTML/CSS renderer at all, see "PDF report" section) |
...
| Cancel still doesn't stop the job, even with the widget's new Cancel button and all 6 backend checkpoints confirmed deployed | `_phase()` unconditionally wrote `status: "processing"` on every call — if a `/cancel-job` request landed and set `status: "cancelled"` right before the pipeline's *next* `_phase()` call, that call silently overwrote it back to `"processing"`, permanently erasing the signal for every downstream `_is_cancelled()` check | Fixed — `_phase()` now checks `_is_cancelled()` before writing and skips the write entirely if already cancelled, instead of blindly stomping the status. See DEPLOYMENT.md changelog #14 |
| PDF report has overlapping/run-together text (e.g. the currency overview cards) and columns cut off past the right edge of the page | The old xhtml2pdf/HTML+CSS template used `display:flex` in several places (xhtml2pdf has no flexbox support at all — those properties were silently ignored) and left several table columns with no explicit width for xhtml2pdf to auto-calculate, which it didn't do reliably against the actual printable page width | Fixed — `generate_pdf_report()` rewritten directly against ReportLab's Platypus API (no HTML/CSS translation step at all); see "PDF report" section above and DEPLOYMENT.md changelog #15 |
...
- The lazy `import anthropic` / `import google.generativeai as genai` /
  `from reportlab...import ...` inside `run_comparison()` /
...
- `generate_pdf_report()`'s direct ReportLab implementation — do NOT swap it
  back for xhtml2pdf/HTML+CSS; that had no flexbox support and unreliable
  table-width handling, confirmed live as the cause of overlapping currency
  cards and a Status column cut off past the page edge. Also do NOT introduce
  Unicode symbols (✓ ⚠ ↔ etc.) into any Paragraph text — base-14 PDF fonts
  don't have those glyphs and silently drop them; use the ASCII equivalents
  already used throughout (e.g. `<->` for a bidirectional arrow)
...
- The `time.sleep(2)` right after the "Fetching quote from Zoho..." and
  "Downloading PDF attachments..." `_phase()` calls — deliberate, not leftover
...
- The `_is_cancelled()` checkpoints scattered through `_run_analysis_pipeline`
  ...
  changelog #13.
- The `if _is_cancelled(context, job_id): return` guard at the top of
  `_phase()` — do NOT remove it to "simplify" back to an unconditional write.
  Without it, `_phase()` silently overwrites a `"cancelled"` status back to
  `"processing"` the moment the pipeline reaches its next phase transition
  after a cancel lands, permanently erasing the signal — confirmed as the
  actual live bug even with all 6 checkpoints and a redeployed widget. See
  DEPLOYMENT.md changelog #14.
